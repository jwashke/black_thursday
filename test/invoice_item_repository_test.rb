...
class InvoiceItemRepositoryTest < MiniTest::Test
  ...
  def test_it_can_return_an_array_of_all_invoice_items
    assert_equal @invoice_items, @invoice_item_repo.all
  end

  def test_it_can_return_an_invoice_item_by_id
    assert_equal @invoice_items[0], @invoice_item_repo.find_by_id(1)
  end
  ...
  def test_it_can_return_all_invoice_items_with_matching_item_id
    invoices = [@invoice_items[0], @invoice_items[1]]
    assert_equal invoices, @invoice_item_repo.find_all_by_item_id(2)
  end
  ...
  def test_it_can_return_all_invoice_items_with_matching_invoice_id
    invoices = [@invoice_items[1], @invoice_items[2]]
    assert_equal invoices, @invoice_item_repo.find_all_by_invoice_id(4)
  end
